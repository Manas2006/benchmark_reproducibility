...
def main():
    ...
    print(f"📁 [DEBUG] Filesystem resolved. Output dir: {output_dir}")

    max_model_length = args.max_model_length
    if args.max_model_length is None:
        print("⚠️ [DEBUG] max_model_length not set, defaulting to max_new_tokens")
        max_model_length = args.max_new_tokens
    elif args.max_model_length == -1:
        print("⚠️ [DEBUG] max_model_length = -1 → setting to None")
    ...
    print(f"📝 [DEBUG] Final output path: {fpath}")

    if fs.exists(fpath) and not args.overwrite:
        print(f"🚫 [DEBUG] Output file already exists and overwrite=False. Exiting early.")
        return
    ...
    if args.system_prompt is not None and os.path.exists(args.system_prompt):
        with open(args.system_prompt, "r") as f:
            ...
            print(f"📜 [DEBUG] Loaded system prompt from {args.system_prompt}")

    env_config = EnvConfig()
    print("🌎 [DEBUG] EnvConfig initialized")
    ...
    print("📊 [DEBUG] EvaluationTracker initialized")
    ...
    print(f"⚙️ [DEBUG] PipelineParameters initialized with:\n{pipeline_params}")
    ...
    print("🚀 [DEBUG] Pipeline created. Starting evaluation...")

    pipeline.evaluate()
    print("✅ [DEBUG] Evaluation complete.")

    pipeline.show_results()
    print("📈 [DEBUG] Results displayed.")

    results = pipeline.get_results()
    print("📦 [DEBUG] Raw results fetched.")

    pipeline.save_and_push_results()
    print("💾 [DEBUG] Results saved.")

    print("✅ Finished one run. Exiting early.")
    data = {
    ...
    print("🧾 [DEBUG] Final result object:")
